...
class Login(object):
    ...
    def siginUp_interface(root):
        signinWindow = tkinter.Toplevel()
        signinWindow.geometry('450x300')
        root.label_username = tkinter.Label(signinWindow,text = 'username')
        root.input_username = tkinter.Entry(signinWindow,width = 30)
        #root.canvas1.pack()
        root.label_username.place(x=60, y=170)
        root.input_username.place(x=150,y=170)
